...
def create_app(config_class=Config):
    ...
    if not google_api_key:
        ...
        try:
            ...
            elapsed = time.time() - start_time
            print(f"[RAG] ❌ Failed to initialize RAG service after {elapsed:.2f}s")
            print(f"[RAG] Error: {str(e)}")
            print(f"[RAG] Traceback:\n{traceback.format_exc()}")
            app.config['RAG_SERVICE'] = None
        ...
        try:
            ...
            elapsed = time.time() - start_time
            print(f"[ANALYTICS] ❌ Failed to initialize Analytics service after {elapsed:.2f}s")
            print(f"[ANALYTICS] Error: {str(e)}")
            print(f"[ANALYTICS] Traceback:\n{traceback.format_exc()}")
            app.config['ANALYTICS_SERVICE'] = None
